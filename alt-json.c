...
int handle_access(void *cls, struct MHD_Connection *connection, const char *url,
  ...
  if (strcmp(url, "/hello") == 0) {
    response =
        MHD_create_response_from_buffer(map_size, map, MHD_RESPMEM_MUST_COPY);
...
int main(int argc, char *argv[]) {
  ...
  daemon = MHD_start_daemon(flags, PORT, NULL, NULL, &handle_access, NULL,
                            MHD_OPTION_CONNECTION_LIMIT, 1000,
                            MHD_OPTION_THREAD_POOL_SIZE, 8, MHD_OPTION_END);
  if (NULL == daemon) {
    return 1;
  }
